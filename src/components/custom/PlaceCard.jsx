...
function PlaceCard({ place, slot }) { 
  ...
  return (<Link to={`https://www.google.com/maps/search/?api=1&query=${place.placeName}}`} target='_blank'>
    <div className="flex flex-col w-[80vw] h-[80vh] md:w-[25vw] md:h-[75vh] bg-white m-2 shadow-lg rounded-2xl transition-transform hover:scale-105 overflow-hidden">
  ...
  <div className="flex flex-col flex-grow  p-4">
    <h2 className="text-xl font-bold text-gray-900 mt-1">{place.placeName}</h2>
        ...
        <p className="text-md text-blue-800 mt-2">💰 {place.ticketPricing}</p>
    <div className="flex-grow"></div>
    <div className="w-full bg-red-50 text-white p-1 flex justify-around rounded-b-2xl">
    <p className="font-medium text-red-800">⏱️ {slot}</p>
    </div>
  </div>
</div>
